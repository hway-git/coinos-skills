...
export type MarketStreamStatus = 'idle' | 'connecting' | 'live' | 'reconnecting' | 'error'
...
export type MarketCandleUpdate = Candle & {
  instrumentId: string
  confirm: boolean
}
...
export type MarketStreamServerMessage =
  | { type: 'status'; channel: 'ticker' | 'candle'; status: MarketStreamStatus; error?: string }
  | { type: 'ticker'; data: MarketTickerUpdate }
  | { type: 'candle'; data: MarketCandleUpdate }
