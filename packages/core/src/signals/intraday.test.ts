...
test('indicator alignment without a PA setup is never actionable', () => {
  ...
  assert.equal(result.strategy?.version, 'helix-pa-expectation/v1')
  assert.equal(result.strategy?.context.alwaysIn, result.signal.bias.side)
  assert.deepEqual(result.strategy?.setups, [])
...
test('a PA setup waits for a later closed-bar break before becoming actionable', () => {
  ...
  assert.equal(waiting.strategy?.context.cycle, 'channel')
  assert.equal(waiting.strategy?.context.direction, 'long')
  assert.equal(waiting.strategy?.setups.length, 2)
  assert.equal(waiting.strategy?.setups[0].state, 'armed')
  assert.equal(waiting.strategy?.setups[0].signalBar.quality, 'good')
  assert.ok((waiting.strategy?.setups[0].signalBar.time ?? 0) > 0)
  assert.ok(Number.isFinite(waiting.strategy?.setups[0].invalidation.price))
  ...
  assert.equal(result.strategy?.selectedTimeframe, '15m')
  assert.equal(result.strategy?.setups[0].state, 'confirmed')
