...
const LEGACY_WATCHLIST_FILE = join(HELIX_REPO_ROOT, 'app', 'dashboard', '.helix-data', 'watchlist.json')
...
async function readWatchlistRecordAt(path: string): Promise<WatchlistRecord | null> {
  try {
    const parsed = JSON.parse(await readFile(path, 'utf8')) as Partial<WatchlistRecord>
...
async function readWatchlistRecord(): Promise<WatchlistRecord | null> {
  const current = await readWatchlistRecordAt(WATCHLIST_FILE)
  if (current || process.env.HELIX_DASHBOARD_DATA_DIR) return current

  const legacy = await readWatchlistRecordAt(LEGACY_WATCHLIST_FILE)
  if (!legacy) return null
  return writeWatchlistRecord(legacy.instruments, legacy.updatedAt)
}

async function writeWatchlistRecord(instruments: string[], updatedAt = Date.now()) {
