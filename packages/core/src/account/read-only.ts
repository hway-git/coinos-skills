...
import type { AccountSnapshot, AccountTableRow } from '@helix/contracts/account'
import { HELIX_REPO_ROOT } from '../runtime-paths'

const execFileAsync = promisify(execFile)
const DEFAULT_TIMEOUT_MS = 18_000
const ACCOUNT_SKILL_DIR = resolve(HELIX_REPO_ROOT, 'skills', 'helix-account')
...
export async function getAccountReconciliationState(): Promise<ActionResult<AccountReconciliationState>> {
  ...
  const normalizedPositions = Array.isArray(positions.data)
    ? positions.data.flatMap((item) => {
      const row = item && typeof item === 'object' ? item as Record<string, unknown> : {}
      const contracts = Math.abs(numberFrom(row.contracts) ?? 0)
      if (contracts <= 0) return []
      const contractSize = numberFrom(row.contractSize) ?? 1
      return [{
        symbol: String(row.symbol ?? '').toUpperCase(),
        side: String(row.side ?? '').toLowerCase(),
        contracts,
        contractSize,
        baseAmount: contracts * contractSize,
      }]
    })
    : []
