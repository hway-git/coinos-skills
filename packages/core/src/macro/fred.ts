...
import type { MacroDataGroup, MacroDataPoint, MacroSnapshot } from '@helix/contracts/market'
...
const SERIES: FredSeries[] = [
  ...
  { id: 'NASDAQQGLDI', label: 'NASDAQ 黄金资金流价格指数', shortLabel: 'Gold Flows', group: 'commodities', unit: 'index', frequency: 'daily', valueKind: 'index', changeKind: 'pct', precision: 2 },
...
function fredApiKey() {
  const configured = process.env.FRED_API_KEY?.trim()
  if (configured) return configured

  try {
    return parseEnv(readFileSync(resolve(homedir(), '.helix', '.env'), 'utf8')).FRED_API_KEY?.trim()
  } catch {
    return undefined
  }
}
