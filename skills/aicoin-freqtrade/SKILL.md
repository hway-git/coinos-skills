...
**单 turn 内不要连发 2+ 个会改 daemon 状态的工具**(例如 create_strategy + set_strategy 一气呵成). OpenClaw / Hermes 的 agent runtime 在单 turn 多步 tool call + 长 streaming 输出时偶发 hang(空响应 — 工具没调成功, 文本也没输出). 改成两轮:
1. 第一轮: 写完策略文件 → 告诉用户"已生成 X.py, 要切上去吗?"
2. 用户确认后第二轮: `set_strategy` 切策略 + 重启 daemon

这条路线对用户也更安全 — 切策略前用户能 review 文件; daemon 重启 ~30s 期间用户对状态有预期, 不会以为卡死.
