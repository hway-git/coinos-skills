...
// 把 Helix 传的参数映射成"正确的 native 订单",ccxt 版本/交易所行为一漂移立刻变红。
//
...
// 退出码: 0 = 全部符合黄金预期; 1 = 有漂移(请核对并更新 GOLDEN 或修 Helix)。
...
// 黄金预期: 每家 × 条件单类型 → 是否应被 ccxt 当成真条件单(true)还是统一参数下不出(false=Helix 安全网会拒)。
...
function pickSwapSymbol(ex) {
  for (const s of ['DOGE/USDT:USDT', 'ETH/USDT:USDT', 'BTC/USDT:USDT']) if (ex.markets[s]) return s;
  return Object.keys(ex.markets).find((s) => typeof s === 'string' && ex.markets[s] && ex.markets[s].swap && s.endsWith(':USDT'));
}
...
async function main() {
  ...
  for (const id of Object.keys(GOLDEN)) {
    ...
    const cases = [
      ['stopLoss', () => build(ex, sym, 'market', 'sell', undefined, { stopLossPrice: dn })],
      ['takeProfit', () => build(ex, sym, 'market', 'sell', undefined, { takeProfitPrice: up })],
      ['trigger', () => build(ex, sym, 'market', 'buy', undefined, { triggerPrice: up, triggerDirection: 'ascending' })],
      ['trailing', () => build(ex, sym, 'market', 'sell', undefined, { trailingPercent: 1, trailingTriggerPrice: last })],
    ];
