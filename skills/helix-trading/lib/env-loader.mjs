...
// (if (!process.env[k]) 守卫)。所以把 key 放进 ~/.helix/.env 后, 旧的 ~/.openclaw
...
// Helix 规范配置文件 —— 跨平台 ~/.helix/.env(Windows: %USERPROFILE%\.helix\.env)。
export function helixEnvFile() {
  return HOME ? join(HOME, '.helix', '.env') : null;
}
...
export function envCandidates() {
  ...
  const helix = helixEnvFile();
  if (helix) list.push(helix);                       // 2. ~/.helix/.env —— 规范位置
...
// saveKey 应写入的 .env 路径 —— 规范位置 ~/.helix/.env(容器内写 /workspace/.env)。
// 调用方写入前需 mkdir -p 父目录(~/.helix 可能还不存在)。
