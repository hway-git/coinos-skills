...
- `walk_forward` rejects censored entries and produces a hash-pinned report from exact reconciled Freqtrade results. A report is promotable only when its pinned versioned policy passes every fee scenario, R-normalized aggregate gate, risk-sizing gate, and strategy-native segment gate. The adapter may use exchange-bounded leverage to fit the `1R = account equity * 0.01` budget inside available stake; actual leverage is part of each reconstructed observation. The policy's reference account equity must drive the historical Freqtrade wallet and be present in the hashed execution profile. Without a policy, treat it as research evidence only.
