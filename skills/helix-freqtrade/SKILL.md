...
- Backtest a Signal Artifact only with its exact `helix.market-dataset/v1` file and a matching observed `helix.futures-cost-dataset/v1`. The decision dataset hash, symbol, market window, provider, and base-timeframe OHLCV must match. The execution-only cost dataset must cover the same source window with continuous 1h mark candles, complete funding rows, and one frozen leverage-tiers snapshot. Never use `futures_funding_rate=0`, last-price candles in place of mark, or a partial exchange response.
...
- `walk_forward` rejects censored entries and produces a hash-pinned single-symbol report from exact reconciled Freqtrade results. It recomputes every trade's funding fee from the pinned funding rate, mark price, amount, direction, and holding window; a reported zero is valid only when this recomputation also yields zero. Under policy v2 that report is only a portfolio member and must fail `SYMBOL_STABILITY_GATE_SATISFIED`; use Core `portfolio-plan` plus `walk_forward_portfolio` to verify the precommitted universe and produce promotion evidence. Every member must use the same frozen leverage-tiers snapshot. The portfolio report pools only valid R statistics and symbol-fold counts; it reports per-symbol and worst-symbol drawdown, never a fabricated shared-capital portfolio drawdown. A report is promotable only when every integrity check and every fee-scenario, funding-cost, risk-sizing, numeric, strategy-native segment, and symbol-stability gate passes. The adapter may use exchange-bounded leverage to fit the `1R = account equity * 0.01` budget inside available stake; the initial Stop price loss plus Entry fee plus estimated Stop-exit fee must fit the trade's `riskR` budget. Actual leverage, price-risk budget, fee-risk budget, and stake are reconstructed for every observation. The policy's reference account equity must drive the historical Freqtrade wallet and be present in the hashed execution profile. Without a policy, treat it as research evidence only.
