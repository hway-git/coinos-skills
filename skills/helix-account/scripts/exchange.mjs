...
const IS_ACCOUNT_SKILL = resolve(__dir, '..').endsWith('helix-account');

function assertTradingAllowed(action) {
  if (IS_ACCOUNT_SKILL) {
    throw new Error(`helix-account 是账户查询 skill，不允许执行 ${action}。请改用 helix-trading skill，并走预览 + 确认流程。`);
  }
}
...
// pionex 在当前 ccxt 版本无实现 → 不列入可交易所。getExchange 另有兜底。
const SUPPORTED = ['binance','okx','bybit','bitget','gate','htx'];

// Exchange display names for prompts and account metadata.
const EXCHANGE_INFO = {
  okx:     { name: 'OKX' },
...
const SECURITY_NOTICE = '⚠️ 交易所 API Key 只保存在本地 .env 或容器 EnvSection 中。不要在 chat、日志或命令行里回显 API secret / passphrase / 私钥。';

// Broker tags override CCXT defaults where an exchange supports them.
const BROKER_CONFIG = {
  binance: {
...
async function getExchange(id, marketType, skipAuth = false) {
  ...
  try {
    ...
    try {
      ...
      throw new Error('ccxt not installed. Run: cd <skill-dir> && npm install');
    }
  }
  const opts = {};
  if (!skipAuth) {
    ...
    if (!opts.apiKey) {
      ...
      throw new Error(
        `未配置 ${info.name || id} 交易所 API Key。` +
        `\n配置方法：把 key 放进 ~/.helix/.env（${pre}_API_KEY=xxx / ${pre}_API_SECRET=xxx），或让 AI 用 save_key 动作代写（自动 chmod 600、不回显）。` +
        `\n${SECURITY_NOTICE}`
      );
    }
  }
  ...
  // Apply broker tags where supported.
...
// 处理: OKX 单向 posSide、币安/Bybit 双向(hedge)缺方向参数。
...
cli({
  exchanges: async () => ({
    supported: SUPPORTED.map(id => {
      const info = EXCHANGE_INFO[id] || {};
      return { exchange: id, name: info.name || id, trading_supported: true };
    }),
    security_notice: SECURITY_NOTICE,
  }),
  register: async ({ exchange: exName }) => {
    if (!exName) return { exchanges: Object.keys(EXCHANGE_INFO), usage: 'node exchange.mjs register \'{"exchange":"okx"}\'' };
    const key = exName.toLowerCase().replace(/[.\s]/g, '');
    const ALIASES = { 币安: 'binance', 火币: 'htx', 派网: 'pionex', gateio: 'gate' };
    const id = ALIASES[key] || key;
    const info = EXCHANGE_INFO[id];
    if (!info) return { error: `不支持 ${exName}`, supported: Object.keys(EXCHANGE_INFO) };
    return {
      exchange: info.name,
      registration: '请前往交易所官网或官方 App 注册。',
      steps: ['打开交易所官网或官方 App 的注册入口', '选择手机或邮箱注册', '填入验证码、设置密码', '完成身份验证(KYC)', '如需 API 交易，到 API 管理创建 key，并通过 Helix 安全配置流程写入 .env'],
      security_notice: SECURITY_NOTICE,
    };
  },
  // 本地 host 模式: 用户在 chat 里给了交易所 key 时, 把它写进规范位置 ~/.helix/.env
  ...
  set_stop: async ({ exchange, symbol, market_type, stop_loss, take_profit, trigger_price, amount, side, force, confirmed }) => {
    ...
    return {
      ...
      风险提示: '⚠️ 条件单是交易所服务器端触发的市价单,触发时按当时市价成交,极端波动可能滑点。ccxt 跨所行为有差异,挂单后务必用 stop_orders 或交易所 APP 复核确实挂上了。',
  ...
  cancel_order: async ({ exchange, symbol, order_id, market_type }) => {
    ...
    const out = {};
    // 不支持一键全撤的所别把 NotSupported 吞成假成功 —— 那会留下没撤掉的单。改 fetchOpenOrders 拉出来逐个撤。
