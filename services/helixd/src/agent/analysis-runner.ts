...
import { getAgentMarketContext } from './market-context'
...
export async function runScheduledAgentAnalysis(
  ...
  try {
    ...
    const [story, marketContext, userMemories] = await Promise.all([
      Promise.resolve(readMarketStory(input)),
      getAgentMarketContext(input),
